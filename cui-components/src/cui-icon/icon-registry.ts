...
export class CuiIconRegistry {
	...
	getSvgIconFromUrl (safeUrl: SafeResourceUrl): Observable<SVGElement> {
		...
		return this._loadSvgIconFromConfig(new SvgIconConfig(safeUrl)).pipe(
			tap(svg => this._cachedIconsByUrl.set(url, <SVGElement>svg)),
	...
	private _getSvgFromConfig (config: SvgIconConfig): Observable<SVGElement> {
		...
		return this._loadSvgIconFromConfig(config).pipe(
			tap(svg => config.svgElement = <SVGElement>svg),
				map(svg => cloneSvg(<SVGElement>svg)),
		);
	}
	...
	private _getSvgFromIconSetConfigs (name: string, iconSetConfigs: SvgIconConfig[]):
		...
		.map(iconSetConfig => this._loadSvgIconSetFromConfig(iconSetConfig).pipe(
	...
	private _loadSvgIconFromConfig (config: SvgIconConfig): Observable<SVGElement> {
		...
		.pipe(map(svgText => this._createSvgElementForSingleIcon(<string>svgText)));
	}
	...
	private _loadSvgIconSetFromConfig (config: SvgIconConfig): Observable<SVGElement> {
		...
		return this._fetchUrl(config.url).pipe(
			map(svgText => this._svgElementFromString(<string>svgText),
			   ));
	}
	...
	private _extractSvgIconFromSet (iconSet: SVGElement, iconName: string): SVGElement | null {
		...
		if (iconNode.tagName.toLowerCase() === 'svg') {
			return this._setSvgAttributes(<SVGElement>iconNode.cloneNode(true));
		}
...
export function ICON_REGISTRY_PROVIDER_FACTORY (
	parentRegistry: CuiIconRegistry, httpClient: HttpClient, sanitizer: DomSanitizer) {
